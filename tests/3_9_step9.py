...
Ваша задача переписать последнюю рабочую версию функции reserve_table (задача Резервация столов: изменение требований - 2) так, чтобы она создавала поле для хранения заказа (ключ «order» со значением пустой словарь). Вот такая структура должна получаться после резервации стола.
