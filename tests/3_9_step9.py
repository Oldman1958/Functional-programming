...
Настало время в нашем ресторане добавить возможность делать заказ посетителям ресторана.
Они могут выбрать любую позицию из меню по следующим категориям:
...
(задача Резервация столов: изменение требований - 2) так,
чтобы она создавала поле для хранения заказа (ключ «order» со значением пустой словарь).
...
Еще может быть такая ситуация, что человек заказывает еду не из перечисленных выше категорий
или просит оказать услугу в рамках заказа.
...
Возможно понадобится продублировать функции, от которых зависела работа перечисленных ранее функций.
